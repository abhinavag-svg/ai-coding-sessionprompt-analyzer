...
5. [Report Structure](#5-report-structure)
6. [Flags Must Link to Dimensions](#6-flags-must-link-to-dimensions)
...
The analyzer produces three layers of output that work together:
...
|---|---|---|---|
| Prompt Clarity | `specificity` | 25% | Prompt concreteness (file paths, symbols, acceptance criteria) — penalized only when vagueness caused a correction |
| Context Efficiency | `context_scope` | 30% | Redundant context: incremental tokens/turn, over-40k ratio, repeated file reads, constraint re-injection frequency |
| Rework Rate | `correction_discipline` | 15% | Prompting habits: prompt-induced rework + repeated constraints across turns |
| AI Consistency | `model_stability` | 10% | Claude failure signals: model-induced rework + unknown rework |
| Task Completion | `session_convergence` | 20% | Session arc: turns to first stable execution, correction spirals, abandonment |

**Weight rationale:** Specificity: 25%, Context Efficiency: 30%, Rework Rate: 15%, AI Consistency: 10%, Task Completion: 20%. Rework Rate and AI Consistency split the 25% correction budget, with Context Efficiency and Prompt Clarity carrying the remaining 55%.
...
**Session shapes:**
- `Efficient` — fast execution, 0-1 correction loops, converged
- `Exploratory` — many turns before first tool use
- `High Rework` — 3+ correction loops mid-session
...
### 5.1 Project-Level Report

The redesigned report emphasizes actionable insights over technical scores:

```
Project Report
│
├── Session Health              ← Overall score, sessions analyzed, spend, recoverable cost, cache savings
├── What to Fix                 ← Flags ranked by recoverable cost, plain-English labels, remedies
├── Score Breakdown             ← Dimension scores with display names + "what it measures" explanations
├── Session Breakdown           ← Ranked table of sessions with top issue per session
├── Prompts That Cost Most      ← Expensive prompts annotated with "why it's expensive" + better version
├── Your Best Prompts           ← Exemplar prompts with "why it worked" explanation
└── Technical Details           ← Token metrics in collapsed <details> block
```

Key design principle: **The composite score earns attention. Dimension scores direct it. Flags explain it. Remedies close the loop.**

### 5.2 Dimension Naming in Reports

Display names are used throughout the report for accessibility:

| Internal Key | Display Name | Example Output |
|---|---|---|
| `specificity` | Prompt Clarity | "Prompt Clarity — Did your prompts tell Claude exactly what to do?" |
| `context_scope` | Context Efficiency | "Context Efficiency — Did you send the right amount of context per turn?" |
| `correction_discipline` | Rework Rate | "Rework Rate — How often did Claude have to redo things?" |
| `model_stability` | AI Consistency | "AI Consistency — Did the model stay reliable and focused?" |
| `session_convergence` | Task Completion | "Task Completion — Did sessions reach a clear, productive end?" |

---

## 6. Flags Must Link to Dimensions
...
- **Efficiency scoring** — Five dimensions measuring different cost drivers (Prompt Clarity, Context Efficiency, Rework Rate, AI Consistency, Task Completion)
- **Anti-pattern detection** — Deterministic JSONL-based pattern matching with 12+ named anti-patterns
- **Cost analysis** — Caching metrics, recoverable cost estimation, cost source transparency
- **Actionable remedies** — Each flag linked to a concrete fix with estimated savings
- **Session lineage** — Foundation for arc modeling and convergence detection
- **Deduplication** — Event deduplication by request/response IDs
...
| Metric | Use | Notes |
|---|---|---|
| incremental tokens/turn | Context Efficiency scoring | Primary cost signal |
...
| correction ratio | Rework Rate + AI Consistency | Split by cause — don't report as one number |
| repeated constraints | Rework Rate | Count phrases recurring 3+ turns |
| file read counts | Context Efficiency — repeated reads only | Total reads not penalized; same-file repeats are |
| turns-to-convergence | Task Completion | Turns from first prompt to first stable tool execution |
| session shape | Task Completion | Efficient / Exploratory / High Rework / Abandoned |
...
- **Model selection** — architectural choice; not within scope of this tool.

---

## 10. Success Criteria

The analyzer works if:

- Every score deduction traces to a named anti-pattern with a concrete remedy
- Rework Rate and AI Consistency point to different interventions
- Task Completion catches correction spirals and abandoned sessions from JSONL alone
...
Decisions made during development, recorded for traceability. Unresolved items are candidates for future work.

---

### 11.1 Multi-Dimension Anti-Pattern Attribution
...
### 11.2 Defining Stable Execution for Session Convergence
...
### 11.3 Recoverable Cost Attribution
...
### 11.4 Correction Turn Detection — Precision vs Recall
...
### 11.5 Token Caps for Reactive Turns
...
### 11.6 Prompt Eligibility Filter
...
### 11.7 error_dump Weighted Noise Classifier
